...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // GET /api/sea/seed-vessels - Seed vessel types
  app.post("/api/sea/seed-vessels", authMiddleware, async (req: AuthRequest, res) => {
    try {
      console.log("[Sea] Seeding MSC vessel types...");
      const { seedSeaVessels } = await import("./seeds/seaVessels");
  ...
  app.get("/api/sea/vessel-types", authMiddleware, async (req: AuthRequest, res) => {
    try {
  ...
  app.get("/api/sea/vessel-types/:id", authMiddleware, async (req: AuthRequest, res) => {
    try {
      const { id } = req.params;
      const [vesselType] = await db.select().from(seaVesselTypes).where(eq(seaVesselTypes.id, parseInt(id)));
      if (!vesselType) {
  ...
  app.get("/api/sea/voyages", authMiddleware, async (req: AuthRequest, res) => {
    try {
  ...
  app.get("/api/sea/voyages/:id", authMiddleware, async (req: AuthRequest, res) => {
    try {
      const { id } = req.params;
      const [voyage] = await db.select().from(seaVoyages)
        .where(and(eq(seaVoyages.id, parseInt(id)), eq(seaVoyages.user_id, req.user!.id)));
      ...
      const containers = await db.select().from(seaContainers)
        .where(eq(seaContainers.voyage_id, parseInt(id)));
  ...
  app.post("/api/sea/voyages", authMiddleware, async (req: AuthRequest, res) => {
    try {
      const voyageData = {
        ...req.body,
        user_id: req.user!.id,
  ...
  app.put("/api/sea/voyages/:id", authMiddleware, async (req: AuthRequest, res) => {
    try {
      const { id } = req.params;
  ...
  app.put("/api/sea/voyages/:id/status", authMiddleware, async (req: AuthRequest, res) => {
    try {
      const { id } = req.params;
      const { status } = req.body;
  ...
  app.delete("/api/sea/voyages/:id", authMiddleware, async (req: AuthRequest, res) => {
    try {
      const { id } = req.params;
  ...
  app.get("/api/sea/containers", authMiddleware, async (req: AuthRequest, res) => {
    try {
      const { voyage_id } = req.query;
      
      let query = db.select().from(seaContainers)
      ...
      if (voyage_id) {
        const containers = await db.select().from(seaContainers)
          .where(and(
            ...
            eq(seaContainers.voyage_id, parseInt(voyage_id as string))
          ))
  ...
  app.get("/api/sea/containers/:id", authMiddleware, async (req: AuthRequest, res) => {
    try {
      const { id } = req.params;
      const [container] = await db.select().from(seaContainers)
        .where(and(eq(seaContainers.id, parseInt(id)), eq(seaContainers.user_id, req.user!.id)));
  ...
  app.post("/api/sea/containers", authMiddleware, async (req: AuthRequest, res) => {
    try {
      const containerData = {
        ...req.body,
        user_id: req.user!.id,
  ...
  app.put("/api/sea/containers/:id", authMiddleware, async (req: AuthRequest, res) => {
    try {
      const { id } = req.params;
  ...
  app.delete("/api/sea/containers/:id", authMiddleware, async (req: AuthRequest, res) => {
    try {
      const { id } = req.params;
  ...
  app.post("/api/sea/containers/:id/assign", authMiddleware, async (req: AuthRequest, res) => {
    try {
      const { id } = req.params;
      ...
      if (voyage_id) {
        const [voyage] = await db.select().from(seaVoyages)
          .where(and(eq(seaVoyages.id, voyage_id), eq(seaVoyages.user_id, req.user!.id)));
      ...
      const [updatedContainer] = await db.update(seaContainers)
        ...
        .where(eq(seaContainers.id, parseInt(id)))
        .returning();
  ...
  app.get("/api/sea/statistics", authMiddleware, async (req: AuthRequest, res) => {
    try {
      const voyages = await db.select().from(seaVoyages)
        .where(eq(seaVoyages.user_id, req.user!.id));
  ...
  app.get("/api/sea/port-schedule", authMiddleware, async (req: AuthRequest, res) => {
    try {
  ...
  app.get("/api/sea/pending-transfers", authMiddleware, async (req: AuthRequest, res) => {
    try {
      const transfers = await db.select({
        transfer: warehouseTransfers,
        source_site: warehouseSites,
  ...
  // POST /api/warehouse/transfers/:id/propose-voyage - Auto-calculate voyage proposal for sea transfer
  app.post("/api/warehouse/transfers/:id/propose-voyage", authMiddleware, async (req: AuthRequest, res) => {
    try {
      const transferId = parseInt(req.params.id);
      const userId = req.user!.id;
  ...
  // POST /api/warehouse/transfers/:id/auto-create-voyage - Auto-create voyage from transfer
  app.post("/api/warehouse/transfers/:id/auto-create-voyage", authMiddleware, async (req: AuthRequest, res) => {
    try {
      const transferId = parseInt(req.params.id);
      const userId = req.user!.id;
  ...
  // POST /api/warehouse/transfers/:id/assign-voyage - Assign existing voyage to transfer
  app.post("/api/warehouse/transfers/:id/assign-voyage", authMiddleware, async (req: AuthRequest, res) => {
    try {
      const transferId = parseInt(req.params.id);
      const { voyage_id } = req.body;
