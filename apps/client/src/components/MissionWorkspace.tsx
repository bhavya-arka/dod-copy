...
import EditableSpreadsheet, {
  SpreadsheetColumn,
  SpreadsheetRow,
} from "./EditableSpreadsheet";
...
export default function MissionWorkspace({
  ...
  const [manifestViewMode, setManifestViewMode] = useState<
    "table" | "spreadsheet"
  >("spreadsheet");
  ...
  const handleManifestDataChange = useCallback(
    (newData: SpreadsheetRow[]) => {
      ...
      const newClassified: ClassifiedItems = {
        ...
        prebuilt_pallets: updatedItems.filter(
          (i) => i.type === "PREBUILT_PALLET",
        ),
  ...
  const handleManifestRowDelete = useCallback(
    (id: string | number) => {
      if (!mission.classifiedItems) return;

      const newClassified: ClassifiedItems = {
        ...mission.classifiedItems,
        rolling_stock: mission.classifiedItems.rolling_stock.filter(
          (i) => (i.item_id || i.utc_id) !== id,
        ),
        prebuilt_pallets: mission.classifiedItems.prebuilt_pallets.filter(
          (i) => (i.item_id || i.utc_id) !== id,
        ),
        loose_items: mission.classifiedItems.loose_items.filter(
          (i) => (i.item_id || i.utc_id) !== id,
        ),
        pax_items: mission.classifiedItems.pax_items.filter(
          (i) => (i.item_id || i.utc_id) !== id,
        ),
        advon_items: mission.classifiedItems.advon_items.filter(
          (i) => (i.item_id || i.utc_id) !== id,
        ),
        main_items: mission.classifiedItems.main_items.filter(
          (i) => (i.item_id || i.utc_id) !== id,
        ),
      };
  ...
  const renderTabContent = () => {
    switch (mission.currentTab) {
                  ...
                  <div className="overflow-x-auto">
                    <table className="w-full">
                      <thead className="bg-neutral-100 border-b border-neutral-200">
                        <tr>
                          <th className="px-4 py-3 text-left text-neutral-700 font-semibold text-sm">
                            Description
                          </th>
                          <th className="px-4 py-3 text-left text-neutral-700 font-semibold text-sm">
                            Length
                      ...
                      <tbody className="divide-y divide-neutral-100">
                        {[
                          ...
                          >
                            ...
                            <td className="px-4 py-3 text-neutral-600 text-sm font-mono">
                              {item.weight_each_lb > 0
                                ? `${item.weight_each_lb.toLocaleString()} lb`
                                : "-"}
                            </td>
                            <td className="px-4 py-3 text-neutral-600 text-sm font-mono">
              ...
              <div className="mt-4 flex items-center justify-between">
                ...
                <div className="text-neutral-500 text-sm">
                  ...
                  {[
                    ...
                    .reduce((sum, item) => sum + item.weight_each_lb, 0)
                    .toLocaleString()}{" "}
                  lb
                </div>
              </div>
              ...
              {mission.splitFlights.length > 0 ? (
                <div className="space-y-3">
                  {mission.splitFlights.map((flight) => (
                    <div
                      key={flight.id}
                      className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2 sm:gap-4 p-3 sm:p-4 bg-neutral-50 rounded-xl border border-neutral-200/50"
                    >
                      <div>
                        <div className="text-neutral-900 font-bold text-sm sm:text-base">
                          {flight.callsign}
                        </div>
                      ...
                      <div className="flex sm:flex-col gap-3 sm:gap-0 sm:text-right text-xs sm:text-sm">
                        <div className="text-green-600 font-mono">
                          DEP:{" "}
                          {flight.scheduled_departure.toLocaleTimeString(
                            "en-US",
                            { hour12: false },
                          )}
                          Z
                        ...
                        <div className="text-primary font-mono">
                          ARR:{" "}
                          {flight.scheduled_arrival.toLocaleTimeString(
                            "en-US",
                            { hour12: false },
                          )}
                          Z
      ...
      case "weather":
        const baseIcaos =
          mission.splitFlights.length > 0
            ? Array.from(
                new Set(
                  mission.splitFlights.flatMap((f) => [
                    f.origin.icao,
                    f.destination.icao,
                  ]),
                ),
          ...
          <div className="p-4 sm:p-6 lg:p-8">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-4">
              {baseIcaos.length > 0 ? (
                baseIcaos.map((icao) => {
                  const base =
                    mission.splitFlights.find((f) => f.origin.icao === icao)
                      ?.origin ||
                    mission.splitFlights.find(
                      (f) => f.destination.icao === icao,
                    )?.destination;
                  if (!base) return null;
                  ...
                  const tempF =
                    weather.temperature_c !== null
                      ? Math.round((weather.temperature_c * 9) / 5 + 32)
                      : null;
                  const conditionBadge =
                    weather.conditions === "VFR"
                      ? "badge-success"
                      : weather.conditions === "MVFR"
                        ? "badge-warning"
                        : "badge-danger";
                    ...
                    <div key={icao} className="glass-card p-4">
                      ...
                      <div className="grid grid-cols-2 gap-2 text-sm">
                        <div>
                          ...
                          {tempF !== null ? (
                            <span className="text-neutral-900 ml-2 font-medium">
                              {tempF}°F
                            </span>
                          ) : (
                        ...
                        <div>
                          ...
                          {weather.ceiling_ft !== null ? (
                            <span className="text-neutral-900 ml-2 font-medium">
                              {weather.ceiling_ft.toLocaleString()} ft
                            </span>
                          ) : (
                            <span className="text-red-500 ml-2 font-medium">
                              N/A
                        ...
                        <div>
                          ...
                          {weather.conditions !== null ? (
                            <span className={`${conditionBadge} ml-2`}>
                              {weather.conditions}
                            </span>
                          ) : (
                            <span className="text-red-500 ml-2 font-medium">
        ...
        return (
          <FlightSplitter
            ...
            existingSplitFlights={
              mission.splitFlights.length > 0 ? mission.splitFlights : undefined
            }
          />
        );
      ...
      case "analytics":
        return (
          <AnalyticsPanel
            onSaveConfiguration={(name: string) =>
              mission.saveConfiguration(name)
            }
          />
        );
