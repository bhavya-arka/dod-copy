...
export default function FlightInsightsPanel({
  ...
  return (
    <aside className={`bg-white/50 backdrop-blur-sm p-4 overflow-auto ${className}`}>
          ...
          >
            ...
            <div className="grid grid-cols-2 gap-2">
              <div className="p-2 rounded-lg bg-blue-50 text-center">
                ...
                <div className="text-lg font-bold text-blue-700">{analysisContent.fleet_status.aircraft_used}</div>
              ...
              <div className="p-2 rounded-lg bg-green-50 text-center">
                ...
                <div className="text-lg font-bold text-green-700">{analysisContent.fleet_status.total_pallets_loaded}</div>
              ...
              <div className="p-2 rounded-lg bg-purple-50 text-center">
                ...
                <div className="text-lg font-bold text-purple-700">{analysisContent.fleet_status.total_pax}</div>
              ...
              <div className="p-2 rounded-lg bg-amber-50 text-center">
                ...
                <div className="text-lg font-bold text-amber-700">{Math.round(analysisContent.fleet_status.average_utilization_percent)}%</div>
                <div className="text-xs text-amber-600">Avg Util</div>
              </div>
            </div>

            {analysisContent.allocation_issues.length > 0 && (
              <div className="space-y-2">
                <h3 className="text-sm font-medium text-neutral-700">Issues & Recommendations</h3>
                {analysisContent.allocation_issues.map((issue, idx) => (
                  <div
                    key={idx}
            ...
            {analysisContent.fleet_shortage_analysis.has_unloaded_cargo && (
              <div className="p-3 rounded-xl bg-red-50 border border-red-200">
                ...
                <p className="text-sm text-red-700 mb-2">
                  {analysisContent.fleet_shortage_analysis.unloaded_item_count} items 
                  ({(analysisContent.fleet_shortage_analysis.unloaded_weight_lb / 1000).toFixed(1)}K lb) 
                  could not be loaded due to insufficient aircraft capacity.
                </p>
                {analysisContent.fleet_shortage_analysis.recommended_additional_aircraft.length > 0 && (
                  <div className="mt-2 space-y-1">
                    <p className="text-xs font-medium text-red-700">Recommended Additional Aircraft:</p>
                    {analysisContent.fleet_shortage_analysis.recommended_additional_aircraft.map((rec, idx) => (
            ...
            {analysisContent.cob_summary.aircraft_out_of_envelope > 0 && (
              <div className="p-3 rounded-xl bg-amber-50 border border-amber-200">
                ...
                <p className="text-sm text-amber-700">
                  {analysisContent.cob_summary.aircraft_out_of_envelope} aircraft out of CoB envelope
                  {analysisContent.cob_summary.worst_offender && ` (worst: ${analysisContent.cob_summary.worst_offender})`}
                </p>
                {analysisContent.cob_summary.corrective_action && (
            ...
            {analysisContent.optimization_notes.length > 0 && (
              <div className="border-t border-neutral-200 pt-3">
                <h3 className="text-sm font-medium text-neutral-700 mb-2">Optimization Notes</h3>
                <ul className="space-y-1">
                  {analysisContent.optimization_notes.map((note, idx) => (
