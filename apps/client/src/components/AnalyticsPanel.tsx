...
export default function AnalyticsPanel({
  ...
  useEffect(() => {
    if (mission.analytics && mission.allocationResult) {
      const feedback = generateQuickInsights(
        mission.analytics,
  ...
  const costBreakdownData = useMemo(() => {
    ...
    return [
      {
        name: "Fuel Cost",
        value: mission.fuelBreakdown.total_fuel_cost_usd,
        color: CHART_COLORS.fuel,
      },
      {
        name: "Operating Cost",
        value: mission.fuelBreakdown.total_operating_cost_usd,
  ...
  const fuelPhaseData = useMemo(() => {
    if (
      !mission.fuelBreakdown ||
      mission.fuelBreakdown.fuel_per_aircraft.length === 0
    )
      return [];

    const totals = mission.fuelBreakdown.fuel_per_aircraft.reduce(
      (acc, aircraft) => ({
        ...
        contingency:
          acc.contingency + aircraft.fuel_breakdown.contingency_fuel_lb,
      }),
  ...
  const selectedAircraftDetails = useMemo(() => {
    ...
    return mission.fuelBreakdown.fuel_per_aircraft.find(
      (a) => a.aircraft_id === selectedAircraftId,
    );
  }, [selectedAircraftId, mission.fuelBreakdown]);

  const cargoTypeData = useMemo(() => {
    if (!mission.allocationResult)
    ...
    for (const plan of plans) {
      palletWeight += plan.pallets.reduce(
        (sum, p) => sum + (p.pallet.gross_weight || 0),
        0,
      );
      rollingStockWeight += plan.rolling_stock.reduce(
    ...
    if (palletWeight > 0)
      data.push({ name: "Palletized", value: palletWeight, color: "#3b82f6" });
    if (rollingStockWeight > 0)
      data.push({
        name: "Rolling Stock",
        value: rollingStockWeight,
        color: "#10b981",
      });
    if (paxWeight > 0)
      data.push({ name: "PAX Weight", value: paxWeight, color: "#f59e0b" });
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 space-y-6">
            ...
            <div className="glass-card p-6">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                <StatCard
                  ...
                  value={mission.analytics.total_aircraft}
                />
                <StatCard
                  label="Total Pallets"
                  value={mission.analytics.total_pallets}
                />
                <StatCard
                  label="Total Weight"
                  value={`${(mission.analytics.total_weight_lb / 1000).toFixed(0)}K lbs`}
                />
                <StatCard
                  label="PAX Count"
                  value={mission.analytics.total_pax}
                />
                <StatCard
                  label="Total Distance"
                  value={`${mission.analytics.total_distance_nm.toLocaleString()} NM`}
                />
                <StatCard
                  label="Flight Hours"
                  value={`${mission.analytics.total_flight_hours.toFixed(1)} hrs`}
                />
                <StatCard
                  label="Avg CoB"
                  value={`${mission.analytics.average_cob_percent.toFixed(1)}%`}
                />
                <StatCard
                  label="Utilization"
                  value={`${mission.analytics.utilization_percent.toFixed(0)}%`}
                />
              </div>
            </div>
          )}
            ...
            <>
              <div className="glass-card p-6">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
                  <div className="stat-card">
                    <div className="stat-value text-blue-600">
                      $
                      {(
                        mission.fuelBreakdown.total_fuel_cost_usd / 1000
                      ).toFixed(0)}
                      K
                    </div>
                  ...
                  <div className="stat-card">
                    <div className="stat-value text-green-600">
                      $
                      {(
                        mission.fuelBreakdown.total_operating_cost_usd / 1000
                      ).toFixed(0)}
                      K
                    </div>
                  ...
                  <div className="stat-card">
                    <div className="stat-value text-purple-600">
                      $
                      {(mission.fuelBreakdown.total_cost_usd / 1000).toFixed(0)}
                      K
                    </div>
                  ...
                  <div className="stat-card">
                    <div className="stat-value text-amber-600">
                      $
                      {mission.fuelBreakdown.average_cost_per_ton_mile.toFixed(
                        2,
                      )}
                    </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div>
                    <h4 className="text-neutral-700 font-medium mb-3 text-sm">
                      Cost Distribution
                    </h4>
              ...
              <div className="glass-card p-6">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
                  ...
                  <div className="stat-card">
                    <div className="stat-value text-blue-600">
                      {mission.fuelBreakdown.average_fuel_efficiency_lb_per_nm.toFixed(
                        1,
                      )}
                    </div>
                  ...
                  <div className="stat-card">
                    <div
                      className={`stat-value ${
                        mission.fuelBreakdown.additional_fuel_from_splits > 0
                          ? "text-amber-600"
                          : "text-green-600"
                      }`}
                    >
                      +
                      {(
                        mission.fuelBreakdown.additional_fuel_from_splits / 1000
                      ).toFixed(0)}
                      K
                    </div>
                ...
                <div className="bg-neutral-50 rounded-xl p-4 mb-4">
                  <div className="flex items-center justify-between text-sm text-neutral-600 mb-2">
                    <span>
                      Reserve Fuel:{" "}
                      {(
                        mission.fuelBreakdown.fuel_config.reserve_fuel_percent *
                        100
                      ).toFixed(0)}
                      %
                    </span>
                    <span>
                      Contingency:{" "}
                      {(
                        mission.fuelBreakdown.fuel_config
                          .contingency_fuel_percent * 100
                      ).toFixed(0)}
                      %
                    </span>
                  </div>
              ...
              <div className="glass-card p-6">
                ...
                <div className="space-y-2 mb-4">
                  {mission.fuelBreakdown.fuel_per_aircraft.map(
                    (aircraft, idx) => (
                      <div
                        key={idx}
                      ...
                      >
                        ...
                        <div className="flex items-center space-x-4 text-sm">
                          <span className="text-neutral-500">
                            {aircraft.distance_nm.toLocaleString()} NM
                          </span>
                          <span className="text-neutral-500">
                            {aircraft.flight_hours.toFixed(1)} hrs
                          </span>
                          <span className="text-primary font-medium">
                            {(
                              aircraft.fuel_breakdown.total_mission_fuel_lb /
                              1000
                            ).toFixed(0)}
                            K lbs
                          </span>
          ...
          {mission.activePlanId && mission.allocationResult && (
            <AiInsightsPanel
              ...
              inputData={{
                ...
                aircraft_types: [
                  ...new Set(
                    mission.allocationResult.load_plans.map(
                      (p) => p.aircraft_type,
                    ),
                  ),
                ],
                total_weight: mission.allocationResult.load_plans.reduce(
                  (sum, p) => sum + p.total_weight,
                  0,
                ),
                total_pallets: mission.allocationResult.load_plans.reduce(
                  (sum, p) => sum + p.pallets.length,
                  0,
                ),
                total_rolling_stock: mission.allocationResult.load_plans.reduce(
                  (sum, p) => sum + p.rolling_stock.length,
                  0,
                ),
                  ...
                  {cargoTypeData.map((item, idx) => (
                    ...
                    >
                      ...
                      <span className="text-neutral-900 font-bold">
                        {(item.value / 1000).toFixed(1)}K lbs
                      </span>
                    </div>
                  ))}
                </div>
                ...
                {aircraftLoadData.map((aircraft, idx) => (
                  <div key={idx} className="bg-neutral-50 rounded-xl p-4">
                    <div className="flex items-center justify-between mb-2">
                      ...
                      <div className="flex items-center space-x-4 text-sm">
                        <span className="text-neutral-500">
                          {(aircraft.weight / 1000).toFixed(1)}K /{" "}
                          {(aircraft.maxPayload / 1000).toFixed(0)}K lbs
                        </span>
                        <span
                          className={`font-bold ${
                            aircraft.loadPercent >= 80
                              ? "text-green-600"
                              : aircraft.loadPercent >= 50
                                ? "text-amber-600"
                                : "text-red-600"
                          }`}
                        >
                    ...
                    <div className="w-full bg-neutral-200 rounded-full h-2">
                      <div
                        ...
                        style={{
                          width: `${Math.min(aircraft.loadPercent, 100)}%`,
                        }}
                      ></div>
                    </div>
          ...
          {palletDistributionData.length > 1 && (
            <div className="glass-card p-6">
              <h3 className="text-neutral-900 font-bold mb-4">
                Pallet Distribution by Aircraft
              </h3>
              <ResponsiveContainer width="100%" height={200}>
          ...
          <div className="glass-card p-6">
            ...
            {mission.savedConfigurations.length === 0 ? (
              <div className="text-center text-neutral-500 py-6">
                ...
                <p className="text-sm">
                  Save your current setup to compare alternatives.
                </p>
              </div>
            ) : (
              <div className="space-y-2">
                {mission.savedConfigurations.map((config) => (
                  <div
                    ...
                    onClick={() =>
                      compareMode
                        ? toggleCompareSelection(config.id)
                        : mission.loadConfiguration(config.id)
                    }
                  >
                    ...
                    <div className="text-neutral-500 text-xs mt-1">
                      {config.split_flights.length} flights •{" "}
              ...
              <div className="bg-blue-50 rounded-lg p-3">
                ...
                <div className="text-blue-700 space-y-1">
                  <div>
                    ...
                    {AIRCRAFT_SPECS[
                      "C-17"
                    ].fuel_burn_cruise_lb_hr.toLocaleString()}{" "}
                    lb/hr
                  </div>
                  <div>
                    Climb:{" "}
                    {AIRCRAFT_SPECS[
                      "C-17"
                    ].fuel_burn_climb_lb_hr.toLocaleString()}{" "}
                    lb/hr
                  </div>
                  <div>
                    Operating: $
                    {AIRCRAFT_SPECS[
                      "C-17"
                    ].operating_cost_per_hr.toLocaleString()}
                    /hr
                  </div>
                </div>
              </div>
...
function AircraftDetailView({ aircraft }: { aircraft: AircraftCostBreakdown }) {
  const fuelPhaseData = [
    {
      name: "Taxi",
      value: aircraft.fuel_breakdown.taxi_fuel_lb,
      color: CHART_COLORS.taxi,
    },
    {
      name: "Climb",
      value: aircraft.fuel_breakdown.climb_fuel_lb,
      color: CHART_COLORS.climb,
    },
    {
      name: "Cruise",
      value: aircraft.fuel_breakdown.cruise_fuel_lb,
      color: CHART_COLORS.cruise,
    },
    {
      name: "Descent",
      value: aircraft.fuel_breakdown.descent_fuel_lb,
      color: CHART_COLORS.descent,
    },
    {
      name: "Reserve",
      value: aircraft.fuel_breakdown.reserve_fuel_lb,
      color: CHART_COLORS.reserve,
    },
    {
      name: "Contingency",
      value: aircraft.fuel_breakdown.contingency_fuel_lb,
      color: CHART_COLORS.contingency,
    },
  ];
          ...
          <div className="space-y-2">
            <div className="flex items-center justify-between bg-white/60 rounded-lg px-3 py-2">
              <div className="flex items-center space-x-2">
                <div
                  className="w-3 h-3 rounded-full"
                  style={{ backgroundColor: CHART_COLORS.fuel }}
                ></div>
            ...
            <div className="flex items-center justify-between bg-white/60 rounded-lg px-3 py-2">
              <div className="flex items-center space-x-2">
                <div
                  className="w-3 h-3 rounded-full"
                  style={{ backgroundColor: CHART_COLORS.operating }}
                ></div>
...
function ConfigurationComparison({
  ...
  configurations: MissionConfiguration[];
}) {
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="text-neutral-500 text-left border-b border-neutral-200">
              ...
              {configurations.map((c) => (
                <th key={c.id} className="py-2 text-neutral-900">
                  {c.name}
                </th>
              ))}
            </tr>
          ...
          <tbody className="text-neutral-900">
            ...
            <tr className="border-b border-neutral-100">
              ...
              {configurations.map((c) => (
                <td key={c.id} className="py-2 font-medium">
                  {c.split_flights.reduce(
                    (sum, f) => sum + f.pallets.length,
                    0,
                  )}
                </td>
              ))}
...
function generateQuickInsights(
  ...
  if (analytics.utilization_percent < 70) {
    insights.push(
      `Aircraft utilization is at ${analytics.utilization_percent.toFixed(0)}%. Consider consolidating cargo to reduce aircraft count and save on operating costs.`,
    );
  } else if (analytics.utilization_percent > 95) {
    insights.push(
      `High utilization (${analytics.utilization_percent.toFixed(0)}%). Weight distribution is efficient, maximizing cost-effectiveness.`,
    );
  }

  if (
    analytics.average_cob_percent < 22 ||
    analytics.average_cob_percent > 32
  ) {
    insights.push(
      `Average center of balance (${analytics.average_cob_percent.toFixed(1)}%) is approaching limits. Review cargo positioning for optimal fuel efficiency.`,
    );
  } else {
    insights.push(
      `Center of balance averaging ${analytics.average_cob_percent.toFixed(1)}% across fleet - well within safe envelope.`,
    );
  }
  ...
  if (fuelBreakdown) {
    if (fuelBreakdown.additional_fuel_from_splits > 0) {
      const additionalCost =
        fuelBreakdown.additional_fuel_from_splits * fuelBreakdown.cost_per_lb;
      insights.push(
        `Splitting cargo adds ${(fuelBreakdown.additional_fuel_from_splits / 1000).toFixed(0)}K lbs of fuel (+$${(additionalCost / 1000).toFixed(0)}K). Evaluate if operational necessity justifies cost.`,
      );
    }

    const fuelPercent =
      (fuelBreakdown.total_fuel_cost_usd / fuelBreakdown.total_cost_usd) * 100;
    if (fuelPercent > 60) {
      insights.push(
        `Fuel accounts for ${fuelPercent.toFixed(0)}% of total mission cost. Consider route optimization or lighter payload distribution.`,
      );
    }

    if (fuelBreakdown.average_cost_per_ton_mile > 0.5) {
      insights.push(
        `Cost per ton-mile ($${fuelBreakdown.average_cost_per_ton_mile.toFixed(2)}) is above average. Maximizing payload per flight could improve efficiency.`,
      );
    } else if (fuelBreakdown.average_cost_per_ton_mile < 0.3) {
      insights.push(
        `Excellent cost efficiency at $${fuelBreakdown.average_cost_per_ton_mile.toFixed(2)} per ton-mile. Current configuration is cost-optimized.`,
      );
    }
  }
  ...
  if (analytics.total_aircraft > 3) {
    insights.push(
      `Large fleet operation with ${analytics.total_aircraft} aircraft. Consider staggered departure times to optimize airspace and ground handling.`,
    );
  }
