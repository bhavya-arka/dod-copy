...
import { validateFlightLoad, calculateFlightWeight, calculateFlightCenterOfBalance } from '../solver/flightSplitUtils';
import { formatMilitaryTime } from '../scheduler/routeCalculations';
...
function generateICODESsvg(loadPlan: AircraftLoadPlan): string {
  ...
  for (let idx = 0; idx < spec.pallet_positions; idx++) {
    const posX = 50 + spec.position_coords[idx] * scale - palletLength / 2;
    ...
    if (placement) {
      ...
      if (placement.pallet.hazmat_flag) {
        palletsSvg += `<text x="${posX + 5}" y="${posY + 10}" fill="#fbbf24" font-size="10">⚠</text>`;
      }
    }
  }
...
function generateLoadPlanHTML(loadPlan: AircraftLoadPlan): string {
  ...
  const palletsRows = loadPlan.pallets.map(p => `
    <tr>
      ...
      <td style="padding: 4px 8px; border-bottom: 1px solid #374151; text-align: center;">${p.pallet.hazmat_flag ? '⚠️ YES' : 'No'}</td>
    </tr>
  `).join('');
    ...
    const insightItems = insights.insights.slice(0, 5).map(insight => {
      ...
      return `
        <div style="background: ${bgColor}; border: 1px solid ${borderColor}; padding: 12px; border-radius: 6px; margin-bottom: 8px;">
          ...
          ${insight.recommendation ? `<p style="margin: 8px 0 0; color: #94a3b8; font-size: 11px; font-style: italic;">💡 ${insight.recommendation}</p>` : ''}
        </div>
      `;
    }).join('');
...
export function exportSessionSummaryToPDF(data: SessionExportData): void {
  ...
  if (data.splitFlights && data.splitFlights.length > 0) {
    const rows = data.splitFlights.map(split => {
      ...
      const recalcCoB = calculateFlightCenterOfBalance(split);
...
export function exportSplitFlightToPDF(splitFlight: SplitFlight): void {
  const recalculatedWeight = calculateFlightWeight(splitFlight);
  const recalculatedCoB = calculateFlightCenterOfBalance(splitFlight);
  ...
  const html = `
    ...
    <body>
      ...
      <div style="display: grid; grid-template-columns: repeat(4, 1fr); gap: 15px; margin-bottom: 20px;">
        ...
        <div style="background: #0f172a; padding: 12px; border-radius: 6px; text-align: center;">
          <p style="margin: 0; color: #94a3b8; font-size: 11px;">Delay</p>
          <p style="margin: 5px 0 0; color: ${splitFlight.estimated_delay_minutes > 0 ? '#f59e0b' : 'white'}; font-weight: bold;">${splitFlight.estimated_delay_minutes > 0 ? '+' + splitFlight.estimated_delay_minutes + ' min' : 'None'}</p>
        </div>
      </div>
      ...
      ${splitFlight.pallets.length > 0 ? `
        <h2 style="color: white; font-size: 16px; margin-bottom: 10px;">Cargo Manifest</h2>
        <table style="width: 100%; border-collapse: collapse; font-size: 12px;">
          <thead>
            <tr style="background: #22c55e;">
      ...
      <footer style="margin-top: 30px; padding-top: 15px; border-top: 1px solid #334155; text-align: center; color: #64748b; font-size: 10px;">
        <p>ARKA CARGO OPERATIONS | ${new Date().toLocaleDateString()}</p>
      </footer>
    </body>
    </html>
  `;
