...
import { calculateFlightWeight, calculateFlightCenterOfBalance } from '../solver/flightSplitUtils';
...
function generateStationData(spec: AircraftSpec, pallets: PalletPlacement[], vehicles: VehiclePlacement[]): ICODESStationData[] {
  ...
  for (let i = 0; i < spec.pallet_positions; i++) {
    ...
    stations.push({
      station_number: i + 1,
      station_location_inches: spec.position_coords[i],
      is_ramp: isRamp,
      occupied: !!placement,
...
export function splitFlightToICODES(flight: SplitFlight, missionId?: string): ICODESLoadPlan {
  ...
  const recalcCoB = calculateFlightCenterOfBalance(flight);
